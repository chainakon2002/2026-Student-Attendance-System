...
function App() {
      ...
      <div className="flex items-center justify-center h-screen bg-slate-50">
        <div className="relative">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <img src="/logo.png" className="w-10 h-10 drop-shadow-md" alt="logo" />
          </div>
        </div>
      </div>
    );
  ...
  // กรณีหน้า Login ไม่ต้องแสดง Navbar และใช้เลย์เอาต์เฉพาะ
  if (!user) {
    return (
      <Router>
        <Routes>
  ...
  return (
    <Router>
      {/* 1. แก้ไขตรงนี้: ลบ overflow-hidden ออกจากกรอบนอกสุด 
        เพื่อไม่ให้มันไปขัดขวางการทำงานของ sticky
      */}
      <div className="min-h-screen bg-[#f8fafc] font-sans relative">
        
        {/* Background Effects (จับมัน Fixed ไว้ข้างหลังสุดแทน จะได้ไม่เลื่อนตาม) */}
        <div className="fixed top-[-10%] left-[-10%] w-[500px] h-[500px] bg-blue-100/60 rounded-full blur-[100px] pointer-events-none z-0"></div>
        <div className="fixed top-[20%] right-[-5%] w-[400px] h-[400px] bg-indigo-100/40 rounded-full blur-[120px] pointer-events-none z-0"></div>

        {/* 2. Container ของ Navbar: 
          - ใช้ sticky top-4 
          - ลบ mb-8 ออกเพื่อป้องกันพื้นที่ขอบล่างดันให้สติกกี้หลุด
        */}
        <div className="sticky top-4 z-50 w-full max-w-7xl mx-auto px-4 sm:px-6 pt-2">
          
          <nav className="relative flex flex-wrap items-center justify-between px-6 py-3 bg-white/50 backdrop-blur-xl border border-white/80 shadow-[0_8px_30px_rgb(0,0,0,0.06)] rounded-3xl transition-all duration-300">
          ...
          {/* Mobile Dropdown */}
          {isMenuOpen && (
            <div className="md:hidden absolute top-20 left-4 right-4 z-40 bg-white/80 backdrop-blur-2xl border border-white/80 rounded-3xl shadow-[0_20px_40px_rgba(0,0,0,0.08)] p-4 text-slate-800 animate-fade-in mt-2">
        ...
        {/* 4. Content Area: เพิ่ม pt-6 เพื่อชดเชยพื้นที่ที่ Navbar ขโมยไป */}
        <div className="relative z-10 container mx-auto p-4 md:px-8 pt-6 pb-20">
          <Routes>
